...
import { StarRating, Icon } from '../ui.jsx';
...
export default function TrackDetail({ track, onClose, onEdit, onDelete }) {
  ...
  return (
    <div onMouseDown={onClose} style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px 16px', background: 'rgba(60,60,90,0.32)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', animation: 'sd-fade .22s ease' }}>
      <div onMouseDown={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 460, background: isDark ? 'rgba(22,22,36,0.97)' : 'rgba(255,255,255,0.95)', borderRadius: 30, border: isDark ? '1px solid rgba(255,255,255,0.12)' : '1px solid rgba(255,255,255,0.9)', boxShadow: isDark ? '0 44px 96px -28px rgba(0,0,0,0.7), inset 0 1px 1px rgba(255,255,255,0.08)' : '0 44px 96px -28px rgba(40,40,80,0.45), inset 0 1px 1px rgba(255,255,255,0.95)', animation: 'sd-pop .26s cubic-bezier(.2,.9,.25,1.1)', overflow: 'hidden' }}>
        ...
        {track.artworkUrl ? (
          <div style={{ position: 'relative', height: 220, overflow: 'hidden' }}>
            ...
            <img src={track.artworkUrl} alt={track.album} style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 130, height: 130, borderRadius: 20, objectFit: 'cover', boxShadow: '0 20px 48px rgba(0,0,0,0.4), 0 0 0 1px rgba(255,255,255,0.3)' }} />
            <ShareCardButton track={track} isDark={isDark} position='overlay' />
            <button onClick={onClose} style={{ position: 'absolute', top: 14, right: 14, display: 'grid', placeItems: 'center', width: 34, height: 34, borderRadius: '50%', background: 'rgba(0,0,0,0.35)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.25)', cursor: 'pointer' }}>
        ...
        <div style={{ padding: '22px 24px 26px' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 16 }}>
            <div style={{ fontSize: 22, fontWeight: 800, color: 'var(--text-1)', letterSpacing: '-0.03em', lineHeight: 1.2 }}>{track.title}</div>
          ...
          {track.previewUrl && (
            <div style={{ marginBottom: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
              <button onClick={togglePlay} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 24px', borderRadius: 999, background: playing ? 'linear-gradient(135deg,#f472b6,#a78bfa)' : 'linear-gradient(135deg,#a78bfa,#818cf8)', border: '1px solid rgba(255,255,255,0.4)', boxShadow: '0 8px 20px rgba(120,90,250,0.35)', cursor: 'pointer', transition: 'all .2s ease' }}>
              ...
              <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 6 }}>
                <div style={{ position: 'relative', height: 5, borderRadius: 999, background: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.08)', overflow: 'hidden' }}>
                  <div style={{ height: '100%', borderRadius: 999, background: 'linear-gradient(90deg, #a78bfa, #f472b6)', width: progress + '%', transition: 'width .1s linear', boxShadow: '0 0 8px rgba(167,139,250,0.5)' }} />
                </div>
          ...
          {track.memo && (
            <div style={{ background: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.03)', borderRadius: 16, border: isDark ? '1px solid rgba(255,255,255,0.1)' : '1px solid rgba(0,0,0,0.07)', padding: '14px 16px', marginBottom: 16 }}>
              <p style={{ fontSize: 14.5, lineHeight: 1.7, color: 'var(--text-2)', fontStyle: 'italic' }}>'{track.memo}'</p>
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={() => { onDelete(track); onClose(); }} style={{ flex: 1, height: 46, borderRadius: 14, fontSize: 14, fontWeight: 600, color: 'rgba(220,50,70,0.9)', background: isDark ? 'rgba(248,113,113,0.12)' : 'rgba(255,90,110,0.08)', border: isDark ? '1px solid rgba(248,113,113,0.25)' : '1px solid rgba(255,90,110,0.2)', cursor: 'pointer' }}>삭제</button>
            <button onClick={() => { onEdit(track); onClose(); }} style={{ flex: 2, height: 46, borderRadius: 14, fontSize: 14.5, fontWeight: 700, color: '#fff', background: 'linear-gradient(135deg, #ff8fcf, #8a7bff 55%, #66d6ff)', border: '1px solid rgba(255,255,255,0.5)', boxShadow: '0 10px 28px -8px rgba(140,60,200,0.5)', cursor: 'pointer' }}>수정하기</button>
          </div>
        </div>
      </div>
    </div>
